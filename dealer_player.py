...
class Player():
    def __init__(self, bet_spread_low, bet_spread_high, num_decks, bank):
        self.p_move = (" ")
        self.bet = 0
        self.bank = bank
    ...
    def player_bet(self, auto, cardCount, cards_used=[]):
        if auto == False:
            while True:
...
class Dealer():
    def __init__(self, num_decks, output):
        self.cards_used = []
        self.the_deck = BasicDeck(num_decks, output)
        self.total_player = []
        self.suits_player = []
        self.suits_dealer = []
        self.total_dealer = []
